...
- `NGINX_PORT` — leave at `80` unless port 80 on the VPS is already taken by
  another app's reverse proxy (check with `sudo ss -tlnp | grep :80` before
  deploying). If it's taken, pick a free port (e.g. `8081`) and make sure
  `APP_URL` includes it too (`http://203.0.113.10:8081`) — otherwise links
  generated by the app (emails, the client upload link) will point at the
  wrong port.
